...
struct LastCategoryManager {
    
    static let shared = LastCategoryManager()
...
extension LastCategoryManager{
    
    func getFirstCategory(completion : @escaping (NetworkResult<Any>) -> (Void) ){
     
        let url = API.baseURL + "/app/last/categories"
    ...
    private func judge(status : Int, data : Data) -> NetworkResult<Any> {
        ...
        guard let decodedData = try? decoder.decode(LastCategoryResponse.self, from : data)
        else {
