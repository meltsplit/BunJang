...
class KaKaoAccessTokenManager{
    static let shared = KaKaoAccessTokenManager()
    
    private var managerID: String{ return String(describing: self)}
    private init(){}
}

extension KaKaoAccessTokenManager{
    
    func checkAccessToken(accessToken : String,completion: @escaping (NetworkResult<Any>) -> Void) {
        
        let url = API.kakaoAutoURL + "/" + accessToken
    ...
    private func judge(status : Int, data : Data) -> NetworkResult<Any> {
        ...
        guard let decodedData = try? decoder.decode(KaKaoAccessTokenResponse.self, from : data)
        else {
