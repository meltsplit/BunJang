...
class MyFollowingManager{
    static let shared = MyFollowingManager()
    
    private var managerID: String{ return String(describing: self)}
    private init(){}
}

extension MyFollowingManager{
    
    func getFollowing(completion: @escaping (NetworkResult<Any>) -> Void) {
        
        
        let url = API.followingURL + "/" + User.shared.userId
    ...
    private func judge(status : Int, data : Data) -> NetworkResult<Any> {
        ...
        guard let decodedData = try? decoder.decode(MyFollowingResponse.self, from : data)
        else {
